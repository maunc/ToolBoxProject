...
fun obtainAppName(): String? {
    val pm: PackageManager = BaseApp.app.packageManager
    try {
        val packageInfo: PackageInfo =
            pm.getPackageInfo(BaseApp.app.packageName, 0)
        val applicationInfo = packageInfo.applicationInfo
        val labelRes = applicationInfo?.labelRes
        return labelRes?.let {
            BaseApp.app.resources.getString(it)
        }
...
fun obtainVersionName(): String? {
    try {
        val packageManager: PackageManager = BaseApp.app.packageManager
        val packageInfo: PackageInfo = packageManager.getPackageInfo(
            BaseApp.app.packageName, 0
        )
...
fun obtainAppVersionCode(): Int {
    ...
    try {
        val pm: PackageManager = BaseApp.app.packageManager
        val pi: PackageInfo = pm.getPackageInfo(BaseApp.app.packageName, 0)
...
fun dp2px(dp: Int): Int =
    (dp * BaseApp.app.resources.displayMetrics.density + 0.5f).toInt()

fun px2dp(px: Int): Int =
    (px / BaseApp.app.resources.displayMetrics.density + 0.5f).toInt()
