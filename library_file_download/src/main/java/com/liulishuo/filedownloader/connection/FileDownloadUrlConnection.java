    ...
    public FileDownloadUrlConnection(URL url, Configuration configuration) throws IOException {
        ...
        if (mConnection instanceof HttpsURLConnection) {
            try {
                ...
                ((HttpsURLConnection) mConnection).setSSLSocketFactory(ctx.getSocketFactory());
                ((HttpsURLConnection) mConnection).setRequestMethod("GET");
    ...
    public void execute() throws IOException {
        if (mConnection instanceof HttpURLConnection) {
            if (FileDownloadLog.NEED_LOG) {
                FileDownloadLog.d(this, "<---- request method %s, connectTimeout %s, readTimeout %s",
                        ((HttpURLConnection) mConnection).getRequestMethod(),
                        ((HttpURLConnection) mConnection).getConnectTimeout(),
                        ((HttpURLConnection) mConnection).getReadTimeout());
            }
        }
        mConnection.connect();
    }
