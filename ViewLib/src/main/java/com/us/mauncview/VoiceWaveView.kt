...
class VoiceWaveView @JvmOverloads constructor(
    ...
    init {
        ...
        attrs?.let {
            ...
            lineWidth = typedArray.getDimension(R.styleable.VoiceWaveView_lineWidth, 15f)
            lineSpace = typedArray.getDimension(R.styleable.VoiceWaveView_lineSpace, 8f)
            duration = typedArray.getInt(R.styleable.VoiceWaveView_duration, 50).toLong()
            showGravity = typedArray.getInt(
                R.styleable.VoiceWaveView_android_gravity,
            ...
            when (absoluteGravity and Gravity.HORIZONTAL_GRAVITY_MASK) {
                Gravity.CENTER_HORIZONTAL -> {
                    ...
                    if (allLineWidth < measuredWidth) {
                        startX =
                            (i * (lineSpace + lineWidth) + lineWidth / 2) + ((measuredWidth - allLineWidth) / 2)
                    } else {
    ...
    fun stop() {
        isStart = false
        valHandler.removeCallbacksAndMessages(null)
